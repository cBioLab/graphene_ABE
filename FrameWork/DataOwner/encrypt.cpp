...
#define USAGE \
  "usage: [ -s scheme ] [ -k master PK ] [ -e encryption input ] [ -i input ] [ -o output ] -v\n\n" \
  "\t-v : turn on verbosity\n"						\
...
void runPkEncrypt(string& suffix, string& sender_id, string& recipient_id, string& inputStr, string& ciphertextFile, bool verbose){
...
void runAbeEncrypt(OpenABE_SCHEME scheme_type, string mpkFile, string& suffix, string& func_input, string& inputStr, string& ciphertextFile, bool verbose){
  OpenABE_ERROR result = OpenABE_NOERROR;
...
int main(int argc, char **argv){
  ...
  bool verbose = false;
  while((opt = getopt(argc,argv,"s:k:i:e:o:r:v")) != EOF){
    switch(opt)
      {
      case 's': scheme_type = string(optarg); break;
      case 'k': mpk_file = string(optarg); break;
  ...
  if(scheme == OpenABE_SCHEME_PK_OPDH){
    ...
    cout << "master PK: " << mpk_file << endl;
    cout << "encryption functional input: "<< func_input << endl;
    runAbeEncrypt(scheme, mpk_file, suffix, func_input, inputStr, ciphertext_file, verbose);
  }
