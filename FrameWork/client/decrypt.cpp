...
#define USAGE \
  "usage: [ -s scheme ] [ -p master PK ] [ -k key ] [ -i ciphertext ] [ -o output ] -v\n\n" \
  "\t-v : turn on verbosity\n"						\
...
int runPkDecrypt(string& suffix, string& sender_id, string& recipient_id, string& ciphertextFile, string& outputFile, bool verbose){
...
int runAbeDecrypt(OpenABE_SCHEME scheme_type, string mpkFile, string& suffix,
...
int main(int argc, char **argv){
  ...
  while((opt = getopt(argc,argv,"e:k:p:s:i:o:r:v")) != EOF){
    switch(opt)
      ...
      case 'p': mpk_file = string(optarg); break;
  ...
  if(scheme_type == OpenABE_SCHEME_PK_OPDH){
    ...
    err_code = runAbeDecrypt(scheme_type, mpk_file, suffix, key_file, ciphertext_file, out_file, verbose);
  }
